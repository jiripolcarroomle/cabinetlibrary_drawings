...
import { Checks } from './lib/internal/checks';
import { createFullOrderOutputByIdFromJson, MultiArticleResult, processMultipleArticles } from './lib/internal/construction_library';
import { clearLogMessages, getLogMessages, logError, LogMessage } from './lib/internal/logging';
import { Dock, PartBase } from './lib/internal/mod-base';
import {
  bomOrderOutputs,
  createOrderOutputByIdFromJson,
  fullOrderOutputs,
  IBomOrderLineData,
  IBomOrderLineDataBase,
...
_global.drawingApi = newDrawingApi();
...
let showDropContainerValue = false;
...
const loadTestOrder = async (): Promise<void> => {
  ...
  bomOrderOutputs.concat(fullOrderOutputs).forEach(bo => {
    const option = document.createElement('option');
    option.text = bo;
    option.value = bo;
...
const doAll = async (): Promise<void> => {
  ...
  articles = processMultipleArticles(json, manMode, true, false, false, showDropContainerValue);
  ...
  articles.articleResults.forEach(p => {
    ...
    p.orderItem?.dockingInfo?.forEach((d, index) => {
      const dock: IVisualPart = {
        ...
        _id: 'docking_' + index,
        _partId: d.id,
        _specialType: d.id === Dock.CollisionBox ? PartSpecialType.CollisionBox : PartSpecialType.DockingVector,
      };
...
const onCreateOrderBomOutput = async (): Promise<void> => {
  ...
  try {
    bomFileResult = await createOrderOutput(orderOutputId);
  } catch (error) {
...
const getDefaultOrderData = (): IOrderData => {
  return {
    orderId: 'ORDER-ID',
    orderNo: 1,
...
const createOrderOutput = async (orderOutputId: string): Promise<Map<string, BomOutputFileEntry>> => {
  console.log('BOM-ORDER OUTPUT: ' + orderOutputId);

  let result = fullOrderOutputs.includes(orderOutputId)
    ? await createFullOrderOutput(orderOutputId)
    : createBomOrderOutput(orderOutputId);
  return result;
};

const createBomOrderOutput = (orderOutputId: string): Map<string, BomOutputFileEntry> => {
  ...
  if (json.hasOwnProperty('orderData')) {
    ...
    if (json && Array.isArray(json) && json.length > 0) {
      ...
      const res = json.find(e => e.hasOwnProperty('rooms'));
      orderData.rooms = res?.rooms;

      const groupPos = json.find(e => e.hasOwnProperty('groupPosition'));
      orderLines.forEach(ol => {
        ol.calcGroupPos = groupPos?.groupPosition;
        ol.calcGroupRotationY = groupPos?.groupRotation[1];
      });
    }
  }
...
const createFullOrderOutput = async (orderOutputId: string): Promise<Map<string, BomOutputFileEntry>> => {
  ...
  const lastUnderscoreIndex = lastOrder.lastIndexOf('_');
  const orderName = lastUnderscoreIndex !== -1
    ? lastOrder.substring(0, lastUnderscoreIndex + 1)
    : lastOrder;

  let orderData: IOrderData | undefined = undefined;
  const orderJsons: any[] = [];
  for (const [key, json] of Object.entries(testOrders)) {
    if (!key.startsWith(orderName) || !json) {
      continue;
    }
    if (json.hasOwnProperty('orderData')) {
      orderData ??= json.orderData;
      if (Array.isArray(json.bomOrderLines)) {
        orderJsons.push(json.bomOrderLines);
      }
    } else if (Array.isArray(json)) {
      orderJsons.push(json);
    }
  }
  ...
  if (!orderData.rooms) {
    const res = orderJsons.flat().find(e => e.hasOwnProperty('rooms'));
    orderData.rooms = res?.rooms;
  }

  const fullOrderLineGroups: IFullOrderLineGroupData[] = [];
  let groupIdx = 0;
  orderJsons.forEach(json => {
    groupIdx++;

    const fullOrderLineGroupData: IFullOrderLineGroupData = {
      groupPos: {},
      items: [],
    }
    let idx = 0;
    json.forEach((e: any) => {
      idx++;
      fullOrderLineGroupData.items.push({
        orderInput: e,
        orderData: {
          orderLineId: 'ORDER-LINE-NO',
          orderLineNo: idx,
          ...
          posArticleNo: 'POS-ARTICLE-NO',
          quantity: 1,
        } as IBomOrderLineData,
      });
    });

    const groupPos = json.find((e: any) => e.hasOwnProperty('groupPosition'));
    if (groupPos) {
      fullOrderLineGroupData.groupPos = {
        calcGroup: groupIdx.toString(),
        calcGroupPos: groupPos.groupPosition,
        calcGroupRotationY: groupPos.groupRotation[1],
      }
    }

    fullOrderLineGroups.push(fullOrderLineGroupData);
  });

  result = await createFullOrderOutputByIdFromJson(orderData, fullOrderLineGroups, orderOutputId);
  return result;
}
...
const updateBomOutputContent = (
  ...
  orderLines: IBomOrderLineDataBase[]
): void => {
...
const onCalculateDropdownEntries = async (): Promise<void> => {
  ...
  else if (result.dropDownRange) {
    const stepText = result.dropDownRange.step ? `, with step ${result.dropDownRange!.step}` : '';
    resultField.innerHTML = `Valid interval: [${result.dropDownRange!.min} - ${result.dropDownRange!.max}]${stepText}`
  }
  else {
    const entriesList = result.dropDownEntries.map(entry => {
      ...
      switch (entry.kind) {
        case 0:
          ...
          break;
        case 1:
          optionValue = "Conflicting";
          break;
        case 2:
          optionValue = "Invalid";
          break;
        default:
          optionValue = "Unknown kind";
          break;
      }
      return `<li>${entry.value}: ${optionValue}</li>`
    }).join('');
    resultField.innerHTML = 'Entries: <ul>' + entriesList + '</ul>';
  }
};
...
const onCalculateConflictingCheck = async (): Promise<void> => {
  ...
  const leadingAttribute = new Checks.CheckAttributeValue();
...
const findModule = (moduleId: string): OD_Base | undefined => {
  function recurse(mod: OD_Base): OD_Base | undefined {
    if (mod._id === moduleId) {
      return mod;
    }
...
const populateAttributeList = (module: OD_Base): void => {
...
document.getElementById('showDropContainerCb')!.addEventListener('change', async ev => {
  const checkedValue = (ev.target as HTMLInputElement).checked;
  showDropContainerValue = checkedValue;
  console.log('Log ~ document.getElementById ~ showDropContainerValue:', showDropContainerValue);
  await doAll();
});
...
document.getElementById('calculateDropdownEntries')!.addEventListener('click', () => {
