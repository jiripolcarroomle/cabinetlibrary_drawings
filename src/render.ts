...
async function createPart(parent: Object3D, part: IVisualPart, showOpen: boolean, showDocking: boolean) {
  ...
  if (part._specialType === PartSpecialType.CollisionBox) {
    if (!showDocking) {
      return;
    }

    const boxGeometry = new BoxGeometry(part._dimx, part._dimy, part._dimz);
    const edgesGeometry = new EdgesGeometry(boxGeometry);
    const basicMaterial = new LineBasicMaterial({ color: 0xff0000, linewidth: 1 });
    const wireframe = new LineSegments(edgesGeometry, basicMaterial);
    grp.add(wireframe);

    const redMaterial = new MeshBasicMaterial({ color: 0xff0000 });
    redMaterial.transparent = true;
    redMaterial.opacity = 0.3;

    const mesh = new Mesh(boxGeometry, redMaterial);
    ...
    grp.add(mesh);
  } else if (part._specialType === PartSpecialType.DockingVector) {
    if (!showDocking) {
      return;
    }
    ...
    if (svgData.paths.length <= 0) {
      logError(
        `SVG data does not contain any paths! Part '${part._partId + ':' + part._id}' will not be drawn! Is the SVG valid? (SVG: ${part._extrude.svg
        })`
      );
    }
