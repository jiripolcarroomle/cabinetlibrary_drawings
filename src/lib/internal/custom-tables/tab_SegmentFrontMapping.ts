...
export interface cti_tab_SegmentFrontMapping {
  readonly in_FrontProgram?: string;
  readonly in_FrontDesign?: string;
  readonly in_FrontColor?: string;
}

export interface ctm_tab_SegmentFrontMapping {
}

export interface cto_tab_SegmentFrontMapping extends ctm_tab_SegmentFrontMapping {
  readonly _id: number;
  readonly SegmentType?: string;
  readonly SegmentColor?: string;
  readonly SegmentGrain?: string;
}

export interface ICT_tab_SegmentFrontMapping
  extends cti_tab_SegmentFrontMapping, cto_tab_SegmentFrontMapping { }

export class ct2_tab_SegmentFrontMapping {

  public findExactly(
    in_FrontProgram: string | undefined,
    in_FrontDesign: string | undefined,
    in_FrontColor: string | undefined,
  ): cto_tab_SegmentFrontMapping | undefined {
    const res = ct_tab_SegmentFrontMapping.find((p) =>
  ...
  public find(
    predicate: (value: cti_tab_SegmentFrontMapping) => boolean
  ): cto_tab_SegmentFrontMapping | undefined {
    for (let index = 0; index < ct_tab_SegmentFrontMapping.length; index++) {
      const element = ct_tab_SegmentFrontMapping[index];
...
export var ct_tab_SegmentFrontMapping: ICT_tab_SegmentFrontMapping[] = [
  {
    _id: 1,
    ...
    in_FrontColor: "All",
    SegmentType: "Glass",
    SegmentColor: "FloatGlass",
    SegmentGrain: "NoGrain"
  }
  , {
    _id: 2,
