...
export interface cti_tab_BaseunitFridgeMapping {
  readonly in_Supplier?: string;
  readonly in_BaseunitFridgeId?: string;
}

export interface ctm_tab_BaseunitFridgeMapping {
}

export interface cto_tab_BaseunitFridgeMapping extends ctm_tab_BaseunitFridgeMapping {
  readonly _id: number;
  readonly ConstructionId?: string;
  readonly GraphicId?: string;
}

export interface ICT_tab_BaseunitFridgeMapping
  extends cti_tab_BaseunitFridgeMapping, cto_tab_BaseunitFridgeMapping { }

export class ct2_tab_BaseunitFridgeMapping {

  public findExactly(
    in_Supplier: string | undefined,
    in_BaseunitFridgeId: string | undefined,
  ): cto_tab_BaseunitFridgeMapping | undefined {
    const res = ct_tab_BaseunitFridgeMapping.find((p) =>
      p.in_Supplier === in_Supplier
      && p.in_BaseunitFridgeId === in_BaseunitFridgeId
    );
    return res;
  }

  public find(
    predicate: (value: cti_tab_BaseunitFridgeMapping) => boolean
  ): cto_tab_BaseunitFridgeMapping | undefined {
    for (let index = 0; index < ct_tab_BaseunitFridgeMapping.length; index++) {
      const element = ct_tab_BaseunitFridgeMapping[index];
...
export var ct_tab_BaseunitFridgeMapping: ICT_tab_BaseunitFridgeMapping[] = [
  {
    _id: 1,
    in_Supplier: "Siemens",
    in_BaseunitFridgeId: "KU20WVHF0",
    ConstructionId: "KU20WVHF0",
    GraphicId: "KU20WVHF0"
  }
  , {
    _id: 2,
    in_Supplier: "Siemens",
    in_BaseunitFridgeId: "KU21RADE0",
    ConstructionId: "KU21RADE0",
    GraphicId: "KU21RADE0"
  }
  , {
    _id: 3,
    in_Supplier: "Siemens",
    in_BaseunitFridgeId: "KU21WAHG0",
    ConstructionId: "KU21WAHG0",
    GraphicId: "KU21WAHG0"
  }
  , {
    _id: 4,
