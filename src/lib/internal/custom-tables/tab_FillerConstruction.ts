...
export interface cti_tab_FillerConstruction {
  readonly in_Module?: string;
  readonly in_Type?: string;
  readonly in_Direction?: string;
}

export interface ctm_tab_FillerConstruction {
}

export interface cto_tab_FillerConstruction extends ctm_tab_FillerConstruction {
  readonly _id: number;
  readonly CarcaseId1?: string;
  readonly CarcaseId2?: string;
  readonly FrontId?: string;
  readonly CarcaseFitting?: string;
  readonly FrontFitting?: string;
}

export interface ICT_tab_FillerConstruction
  extends cti_tab_FillerConstruction, cto_tab_FillerConstruction { }

export class ct2_tab_FillerConstruction {

  public findExactly(
    in_Module: string | undefined,
    in_Type: string | undefined,
    in_Direction: string | undefined,
  ): cto_tab_FillerConstruction | undefined {
    const res = ct_tab_FillerConstruction.find((p) =>
      p.in_Module === in_Module
      && p.in_Type === in_Type
  ...
  public find(
    predicate: (value: cti_tab_FillerConstruction) => boolean
  ): cto_tab_FillerConstruction | undefined {
    for (let index = 0; index < ct_tab_FillerConstruction.length; index++) {
      const element = ct_tab_FillerConstruction[index];
...
export var ct_tab_FillerConstruction: ICT_tab_FillerConstruction[] = [
  {
    _id: 12,
    in_Module: "mc_FillerStraight01",
    in_Type: "LShape",
    in_Direction: "Left",
    CarcaseId1: "SpShortLeft",
    CarcaseId2: "None",
    FrontId: "Panel",
    CarcaseFitting: "None",
    FrontFitting: "Pacofix"
  }
  , {
    _id: 13,
    in_Module: "mc_FillerStraight01",
  ...
  , {
    _id: 14,
    in_Module: "mc_FillerStraight01",
    in_Type: "CoverCarcase",
    in_Direction: "Left",
    CarcaseId1: "CoverCarcaseLeft",
    CarcaseId2: "None",
    FrontId: "Panel",
    CarcaseFitting: "Dowel",
    FrontFitting: "Pacofix"
  }
  , {
    _id: 15,
