...
export interface cti_tab_FillerPartConstruction {
  readonly in_ConstructionId?: string;
}

export interface ctm_tab_FillerPartConstruction {
}

export interface cto_tab_FillerPartConstruction extends ctm_tab_FillerPartConstruction {
  readonly _id: number;
  readonly PartReference?: string;
  Width(m: any): number;
  Depth(m: any): number;
  Thickness(m: any): number;
  PosWidth(m: any): number;
  PosHeight(m: any): number;
  PosDepth(m: any): number;
}

export interface ICT_tab_FillerPartConstruction
  extends cti_tab_FillerPartConstruction, cto_tab_FillerPartConstruction { }

export class ct2_tab_FillerPartConstruction {

  public findExactly(
    in_ConstructionId: string | undefined,
  ): cto_tab_FillerPartConstruction | undefined {
    const res = ct_tab_FillerPartConstruction.find((p) =>
  ...
  public find(
    predicate: (value: cti_tab_FillerPartConstruction) => boolean
  ): cto_tab_FillerPartConstruction | undefined {
    for (let index = 0; index < ct_tab_FillerPartConstruction.length; index++) {
      const element = ct_tab_FillerPartConstruction[index];
...
export var ct_tab_FillerPartConstruction: ICT_tab_FillerPartConstruction[] = [
];
