...
export interface ICT_tab_HoodAssemblyMapping
  extends cti_tab_HoodAssemblyMapping, cto_tab_HoodAssemblyMapping { }
...
export class ct2_tab_HoodAssemblyMapping {
  ...
  public find(
    predicate: (value: cti_tab_HoodAssemblyMapping) => boolean
  ): cto_tab_HoodAssemblyMapping | undefined {
    for (let index = 0; index < ct_tab_HoodAssemblyMapping.length; index++) {
      const element = ct_tab_HoodAssemblyMapping[index];
