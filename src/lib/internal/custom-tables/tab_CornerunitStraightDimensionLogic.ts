...
import { ITableParents_mr_CornerunitStraight } from '../var-interfaces';

export interface cti_tab_CornerunitStraightDimensionLogic {
  readonly in_DimensionLogic?: string;
  readonly in_CarcaseDirection?: string;
}

export interface ctm_tab_CornerunitStraightDimensionLogic {
}

export interface cto_tab_CornerunitStraightDimensionLogic extends ctm_tab_CornerunitStraightDimensionLogic {
  readonly _id: number;
  mod_Width(m: ITableParents_mr_CornerunitStraight): number;
  mod_TotalWidth(m: ITableParents_mr_CornerunitStraight): number;
  mod_WidthBlind(m: ITableParents_mr_CornerunitStraight): number;
  mod_WidthRight(m: ITableParents_mr_CornerunitStraight): number;
  mod_CornerunitFrontWidth(m: ITableParents_mr_CornerunitStraight): number;
  mod_Depth(m: ITableParents_mr_CornerunitStraight): number;
  mod_TotalDepth(m: ITableParents_mr_CornerunitStraight): number;
  mod_WidthLeft(m: ITableParents_mr_CornerunitStraight): number;
}

export interface ICT_tab_CornerunitStraightDimensionLogic
  extends cti_tab_CornerunitStraightDimensionLogic, cto_tab_CornerunitStraightDimensionLogic { }

export class ct2_tab_CornerunitStraightDimensionLogic {

  public findExactly(
    in_DimensionLogic: string | undefined,
    in_CarcaseDirection: string | undefined,
  ): cto_tab_CornerunitStraightDimensionLogic | undefined {
    const res = ct_tab_CornerunitStraightDimensionLogic.find((p) =>
  ...
  public find(
    predicate: (value: cti_tab_CornerunitStraightDimensionLogic) => boolean
  ): cto_tab_CornerunitStraightDimensionLogic | undefined {
    for (let index = 0; index < ct_tab_CornerunitStraightDimensionLogic.length; index++) {
      const element = ct_tab_CornerunitStraightDimensionLogic[index];
...
export var ct_tab_CornerunitStraightDimensionLogic: ICT_tab_CornerunitStraightDimensionLogic[] = [
];
