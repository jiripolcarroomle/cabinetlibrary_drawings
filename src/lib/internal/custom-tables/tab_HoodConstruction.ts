...
export interface ICT_tab_HoodConstruction
  extends cti_tab_HoodConstruction, cto_tab_HoodConstruction { }
...
export class ct2_tab_HoodConstruction {
  ...
  public find(
    predicate: (value: cti_tab_HoodConstruction) => boolean
  ): cto_tab_HoodConstruction | undefined {
    for (let index = 0; index < ct_tab_HoodConstruction.length; index++) {
      const element = ct_tab_HoodConstruction[index];
