...
import { ITableParents_mc_HoodCarcaseParts01 } from '../var-interfaces';

export interface cti_tab_HoodAssemblyParts {
  readonly in_CarcaseConstructionID?: string;
}

export interface ctm_tab_HoodAssemblyParts {
}
...
export interface ICT_tab_HoodAssemblyParts
  extends cti_tab_HoodAssemblyParts, cto_tab_HoodAssemblyParts { }

export class ct2_tab_HoodAssemblyParts {

  public findExactly(
    in_CarcaseConstructionID: string | undefined,
  ): cto_tab_HoodAssemblyParts | undefined {
    const res = ct_tab_HoodAssemblyParts.find((p) =>
  ...
  public find(
    predicate: (value: cti_tab_HoodAssemblyParts) => boolean
  ): cto_tab_HoodAssemblyParts | undefined {
    for (let index = 0; index < ct_tab_HoodAssemblyParts.length; index++) {
      const element = ct_tab_HoodAssemblyParts[index];
