...
import { createFullOrderOutputById, IBomOrderLineData, IFullOrderLineGroupData, IOrderData } from './order_outputs';
...
function loadAndCompleteData(json: any, manufacturerMode: boolean, generationContours?: GenerationContour[], roomContours?: RoomContour[]): OD_Base {
...
function buildRoot(od: OD_Base, withDropContainers?: boolean): void {
    ...
    if (withDropContainers) {
        od.calculateContainerModules();
    }
...
function dataCompletionAndCreateBuildPlan(json: any, manufacturerMode: boolean, generationContours?: GenerationContour[], roomContours?: RoomContour[], withDropContainers?: boolean): OD_Base {
    const od = loadAndCompleteData(json, manufacturerMode, generationContours, roomContours);

    buildRoot(od, withDropContainers);
    return od;
}
...
export function processMultipleArticles(json: any, manMode: boolean, withBom: boolean, withAllOutputs: boolean, withPerformanceLogs: boolean | undefined, withDropContainers: boolean | undefined): MultiArticleResult {
    ...
    // Step 1: dataCompletion + manufacturerDataCompletion for all articles
    const rootModules: OD_Base[] = [];
    json.forEach(o => {
        if (!checkIfModuleWasGenerated(o)) {
            var ares = prepareArticleResult(o, manMode, undefined, ol?.roomContours);
            res.articleResults.push(ares);

            if (ares.orderItem) {
                rootModules.push(ares.orderItem);
            }
        }
    });

    // Step 2: prepareContext for all articles
    ...
    // Step 3: afterDataCompletion + createBuildPlan for all articles
    res.articleResults.forEach(ares => {
        finalizeArticleResult(ares, withDropContainers);
    });
    ...
    if (withBom || withAllOutputs) {
        ...
        const nonContainerParts = allPartArray.filter(p => p._isContainerPart != true);
        const detectedTouches = new CollisionMap();
        const groupedByRotationY = new Map<number, PartBase[]>();
        groupedByRotationY.set(-1, nonContainerParts.filter(p => p.canHaveRotatedTouches));

        groupByRotationY(nonContainerParts.filter(p => !p.canHaveRotatedTouches ||
        ...
        setFullMatrixFor3DElements(allPartArray);
        res.logMessages = getLogMessages();
...
export function prepareArticleResult(o: any, manMode: boolean, generationContours?: GenerationContour[], roomContours?: RoomContour[]): ArticleResult {
    var ares = new ArticleResult();
    try {
        ares.rootId = o.id;  // always set the "id", so we can always correlate this entry with the order item
        var od = loadAndCompleteData(o, manMode, generationContours, roomContours);
...
function finalizeArticleResult(ares: ArticleResult, withDropContainers?: boolean): void {
    setLogMessages(ares.logMessages);
    try {
        if (ares.orderItem) {
            buildRoot(ares.orderItem, withDropContainers);
            partSetAttributes(ares.orderItem);

            // Adjust the group name to also contain the article (root) id
            if (ares.orderItem._partGroups) {
                var remap = new Map<string, PartGroup>();
                var rootId = ares.orderItem._id;
                ares.orderItem._partGroups.forEach((value, key) => {
                    var grpName = rootId + "-" + value._groupName;
                    value._groupName = grpName;
                    remap.set(grpName, value);
                });
                ares.orderItem._partGroups = remap;
            }
        }
    }
    catch (error) {
...
// INFO: This is used by the processGenerationRootModules
function calculateOrderItem(o: any, manMode: boolean, generationContours?: GenerationContour[], roomContours?: RoomContour[], withDropContainers?: boolean): ArticleResult {
    var ares = new ArticleResult();
    var od;
    try {
        ares.rootId = o.id;  // always set the "id", so we can always correlate this entry with the order item
        // INFO: This also clears to log messages
        od = dataCompletionAndCreateBuildPlan(o, manMode, generationContours, roomContours, withDropContainers);
        ares.attrChanges = getAttrChangeLogs();
        ares.orderItem = od;
...
export async function createFullOrderOutputByIdFromJson(o: IOrderData, orderLineGroups: IFullOrderLineGroupData[], orderOutputId: string): Promise<Map<string, BomOutputFileEntry>> {

    if (!Array.isArray(orderLineGroups)) {
        logError("Input 'orderLineGroups' in 'createFullOrderOutputByIdFromJson' is not an array!");
        return Promise.resolve(new Map<string, BomOutputFileEntry>());
    }
    for (const orderLineGroupData of orderLineGroups) {
        if (!Array.isArray(orderLineGroupData.items)) {
            logError("Items in 'orderLineGroupData.items' in 'createFullOrderOutputByIdFromJson' is not an array!");
            continue;
        }

        const inputJson = orderLineGroupData.items.map(item => item.orderInput);
        const articles = processMultipleArticles(inputJson, false, true, false, false, false);
        if (inputJson.length !== articles.articleResults.length) {
            logError("Number of processed articles does not match number of input articles in 'createFullOrderOutputByIdFromJson'!");
            continue;
        }

        for (let i = 0; i < orderLineGroupData.items.length; i++) {
            const orderLineData = orderLineGroupData.items[i].orderData;
            const article = articles.articleResults[i];
            if (article.bom && article.orderItem) {
                article.orderItem.toJson();
                orderLineData.bomEntries = article.bom;
                orderLineData.orderItem = article.orderItem;
            }
        }
    }

    return await createFullOrderOutputById(o, orderLineGroups, orderOutputId);
}


export function checkIfModuleWasGenerated(json: any) {
    ...
    generationGroups.forEach((val, key) => {
        ...
        switch (key) {
            ...
            case GenerationMethod.Countertop:
                ...
                break;
            case GenerationMethod.Backsplash:
                rootModuleId = 'mr_Backsplash';
                break;
            ...
            case GenerationMethod.Fingergrip:
                ...
                break;
            case GenerationMethod.PlinthAreaBaseboard:
                rootModuleId = 'mr_PlinthAreaBaseboard';
                break;
            case GenerationMethod.CeilingFiller:
                rootModuleId = 'mr_CeilingFiller';
                break;
            default:
