...
import { IOutputFileEntry } from './order_outputs';
import { create as xmlbuilder2Create } from 'xmlbuilder2';

export class BomOutputFileEntry implements IOutputFileEntry {
...
class BOM_Output_create_CamManagerProcessings extends BomOutput {
  constructor(part?: PartBase) {
    super(part);
    this.name = 'create_CamManagerProcessings';
    this.bomType = 'NC';
  }
  override createPartOutput(part: PartBase, bomEntries: BOM_ElementTypeBase[]): Map<string, BomOutputFileEntry> {
    let result = new Map<string, BomOutputFileEntry>();
...
export function createBomOutputInstance(part: PartBase, bomOutputId: string): BomOutput {
  ...
  if (bomOutputId.toLowerCase() == 'create_cammanagerprocessings') {
    return new BOM_Output_create_CamManagerProcessings(part)
  }
