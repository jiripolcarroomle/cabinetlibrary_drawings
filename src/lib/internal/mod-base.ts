...
import { OD_Base, PartGroup, OpenGroup, Matrix4, Vector3, FullMatrixMode, ArticlePos } from './base'
...
export interface IModBasePropBase {
  get _id(): string; // Unique moduleId (guid)
  get modId(): string; // module id
  get g(): IGlobalVars;
}
export interface IModBaseProp extends IModBasePropBase {
  getOrigin(): Matrix4;
...
export interface IModBase extends IModBaseProp {
  ...
  get _rootArticleId(): string | undefined;  // article id of the root module
...
export interface IPartBase extends I3DElement {
  ...
  /**
   * Gets the width (in UI length) of the part based on the WDT mapping.
   **/
  get _width(): number;
  /**
   * Gets the depth (in UI width) of the part based on the WDT mapping.
   **/
  get _depth(): number;
  /**
   * Gets the thickness (in UI thickness) of the part based on the WDT mapping.
   **/
...
export abstract class PartBase implements IPartBase {
  constructor(parent: IModBaseProp | undefined, partId: string, x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    ...
    this._isContainerPart = isContainerPart;
  }
  ...
  toJsonOD(): any {
    ...
    json._isContainerPart = this._isContainerPart;
    return json;
  }
...
export interface IDockedContext {
  ownDockingVector: Dock;
  dockedRoots: IDockedContextRoot[];
}

export interface IDockedContextRoot {
  id: string;
  dockingVector: Dock;
}
