...
import { IPartParentsNonNull_mc_CoatBoard_mr_CoatBoard } from '../mod-interfaces'

export interface IP_part_CoatBoardGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_CoatBoard_mr_CoatBoard;
  get pa_BomId(): string;
}

export interface IP_part_CoatBoardGroup_PartVarsWritable extends IP_part_CoatBoardGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_CoatBoardGroup extends PartBase implements IP_part_CoatBoardGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_CoatBoard_mr_CoatBoard,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_CoatBoardGroup', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_CoatBoard_mr_CoatBoard; // module which created this part
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_CoatBoardGroup_ValueScript
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_CoatBoardGroup (id: ' + this._id + ')');
    }
