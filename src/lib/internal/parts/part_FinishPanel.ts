...
export class P_part_FinishPanel extends PartBase implements IP_part_FinishPanel_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Upright01,
    ...
    super(parent, 'part_FinishPanel', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'ZYX';
  }
  ...
  setAttributes() {
    try {
      ...
      if (this.#pa_ProgramGrainGroup === undefined) {
        this.pa_ProgramGrainGroup = this.parent.mod_UprightProgram_matrix.GrainGroupId;
      }
      if (this.#pa_ColorGrainGroup === undefined) {
        this.pa_ColorGrainGroup = this.parent.mod_UprightColor_matrix.GrainGroupId;
      }
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_TypeElement === undefined) {
      logError('Attribute pa_TypeElement not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_ProgramGrainGroup === undefined) {
      logError('Attribute pa_ProgramGrainGroup not set in part part_FinishPanel (id: ' + this._id + ')');
    }
    if (this.#pa_ColorGrainGroup === undefined) {
      logError('Attribute pa_ColorGrainGroup not set in part part_FinishPanel (id: ' + this._id + ')');
    }
