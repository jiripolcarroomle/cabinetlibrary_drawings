...
import { IPartParentsNonNull_mc_PantryPulloutHardware01 } from '../mod-interfaces'

export interface IP_part_PantryPulloutBom_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_PantryPulloutHardware01;
  get pa_HardwareId(): string;
  get pa_ParentName(): string;
}

export interface IP_part_PantryPulloutBom_PartVarsWritable extends IP_part_PantryPulloutBom_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_ParentName(value: string);
}

export class P_part_PantryPulloutBom extends PartBase implements IP_part_PantryPulloutBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_PantryPulloutHardware01,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_PantryPulloutBom', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_PantryPulloutHardware01; // module which created this part
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_PantryPulloutBom_ValueScript
      if (this.#pa_HardwareId === undefined) {
        this.pa_HardwareId = '';
      }
      if (this.#pa_ParentName === undefined) {
        this.pa_ParentName = this.parent.mod_FrontId;
      }
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_PantryPulloutBom (id: ' + this._id + ')');
    }
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_PantryPulloutBom (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM232 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_PantryPulloutBom_PartVarsReadOnly) {
            const part = partSelf;
        ...
        _bom._description = "Insert all the hardware elements to the BOM";
        _bom.set(partSelf);
