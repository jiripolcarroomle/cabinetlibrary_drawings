...
export class P_part_Sidepanelleft extends PartBase implements IP_part_Sidepanelleft_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_StorageunitSidepanel01,
    ...
    super(parent, 'part_Sidepanelleft', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'YZX';
  }
  ...
  setAttributes() {
    try {
      ...
      if (this.#pa_EdgeFrontColor === undefined) {
        this.pa_EdgeFrontColor = this.parent.mod_CarcaseVisLeft
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeFrontColor;;
      }
      if (this.#pa_EdgeLeftColor === undefined) {
        this.pa_EdgeLeftColor = (this.parent.mod_CarcaseVisLeft || this.parent.mod_CarcaseVisTop)
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeColor;;
      }
      if (this.#pa_EdgeBackColor === undefined) {
        this.pa_EdgeBackColor = this.parent.mod_CarcaseVisLeft
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeBackColor;;
      }
      if (this.#pa_EdgeRightColor === undefined) {
        this.pa_EdgeRightColor = (this.parent.mod_CarcaseVisLeft || this.parent.mod_CarcaseVisBtm)
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeColor;;
      }
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
    ...
    try {
      // ncout_Group: 640: part_PulloutHardwareDrillings FromRight
      if (touch == PartTouch.FromRight && partNext instanceof P_part_PulloutHardwareDrillings) {
        let part2 = <P_part_PulloutHardwareDrillings>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT640 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Sidepanelleft_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PulloutHardwareDrillings_PartVarsReadOnly, posRel: CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
          ...
          _bom._parentId = partSelf._id;
          _bom._description = "Drilling for pullout hardware";
          _bom.set(partSelf, touch, part2, posRel);
          partSelf._bom.push(_bom);
          if (_bom._subBom) {
            _bom._subBom.forEach(p => partSelf._bom.push(p));
    ...
    try {
      // ncout_Group: 665: part_ClothingOrganizerDrilling FromRight
      if (touch == PartTouch.FromRight && partNext instanceof P_part_ClothingOrganizerDrilling) {
        let part2 = <P_part_ClothingOrganizerDrilling>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT665 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Sidepanelleft_PartVarsReadOnly, touch: PartTouch, part2: IP_part_ClothingOrganizerDrilling_PartVarsReadOnly, posRel: CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
              try {
                ...
                drillings.forEach(x => {
                  ...
                  let DrillVert = this.addncout_DrillVert();
                  DrillVert.nc_TOOL = "102";
                  DrillVert.nc_XA = partSelf._dimy - PosX;
                  DrillVert.nc_YA = partSelf._dimz - PosY;
                  DrillVert.nc_TI = x.TI;
                  DrillVert.nc_DU = x.DU;
