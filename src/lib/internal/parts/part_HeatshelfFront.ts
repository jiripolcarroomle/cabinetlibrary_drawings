...
export class P_part_HeatshelfFront extends PartBase implements IP_part_HeatshelfFront_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_StorageunitShelftop02,
    ...
    super(parent, 'part_HeatshelfFront', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProgramGrainGroup === undefined) {
      logError('Attribute pa_ProgramGrainGroup not set in part part_HeatshelfFront (id: ' + this._id + ')');
    }
    if (this.#pa_ColorGrainGroup === undefined) {
      logError('Attribute pa_ColorGrainGroup not set in part part_HeatshelfFront (id: ' + this._id + ')');
    }
