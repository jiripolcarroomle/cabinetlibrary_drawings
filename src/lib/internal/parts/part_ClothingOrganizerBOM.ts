...
import { IPartParentsNonNull_mc_ClothingOrganizerHardware01 } from '../mod-interfaces'

export interface IP_part_ClothingOrganizerBOM_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_ClothingOrganizerHardware01;
  get pa_HardwareId(): string;
  get pa_ParentName(): string;
}

export interface IP_part_ClothingOrganizerBOM_PartVarsWritable extends IP_part_ClothingOrganizerBOM_PartVarsReadOnly {
  set pa_HardwareId(value: string);
  set pa_ParentName(value: string);
}

export class P_part_ClothingOrganizerBOM extends PartBase implements IP_part_ClothingOrganizerBOM_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_ClothingOrganizerHardware01,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_ClothingOrganizerBOM', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_ClothingOrganizerHardware01; // module which created this part
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_ClothingOrganizerBOM_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_ClothingOrganizerBOM (id: ' + this._id + ')');
    }
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_ClothingOrganizerBOM (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM236 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_ClothingOrganizerBOM_PartVarsReadOnly) {
            const part = partSelf;
            ...
            try {
              ...
              if (!Array.isArray(hardwareIdElements.values)) {
                throw new Error("Could not create the BOM for the drawerbox fittings");
              }
        ...
        _bom._description = "All the hardware components for the clothing organizer";
        _bom.set(partSelf);
