...
export interface IP_part_ClothingOrganizerDrilling_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_ClothingOrganizerHardware01;
  get pa_ProcessingId(): string;
}

export interface IP_part_ClothingOrganizerDrilling_PartVarsWritable extends IP_part_ClothingOrganizerDrilling_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
}

export class P_part_ClothingOrganizerDrilling extends PartBase implements IP_part_ClothingOrganizerDrilling_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_ClothingOrganizerHardware01,
    ...
    super(parent, 'part_ClothingOrganizerDrilling', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_ClothingOrganizerDrilling_ValueScript
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_ClothingOrganizerDrilling (id: ' + this._id + ')');
    }
