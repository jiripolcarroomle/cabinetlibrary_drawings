...
import { IPartParentsNonNull_mc_MirrorBoard_mr_MirrorBoard } from '../mod-interfaces'

export interface IP_part_MirrorBoardGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_MirrorBoard_mr_MirrorBoard;
  get pa_BomId(): string;
}

export interface IP_part_MirrorBoardGroup_PartVarsWritable extends IP_part_MirrorBoardGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_MirrorBoardGroup extends PartBase implements IP_part_MirrorBoardGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_MirrorBoard_mr_MirrorBoard,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_MirrorBoardGroup', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_MirrorBoard_mr_MirrorBoard; // module which created this part
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_MirrorBoardGroup_ValueScript
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_MirrorBoardGroup (id: ' + this._id + ')');
    }
