...
export class P_part_FlipliftHardwareBom extends PartBase implements IP_part_FlipliftHardwareBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FlipliftHardware01,
    ...
    super(parent, 'part_FlipliftHardwareBom', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  ...
  checkAttributes() {
    ...
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_FlipliftHardwareBom (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareId === undefined) {
      logError('Attribute pa_HardwareId not set in part part_FlipliftHardwareBom (id: ' + this._id + ')');
    }
