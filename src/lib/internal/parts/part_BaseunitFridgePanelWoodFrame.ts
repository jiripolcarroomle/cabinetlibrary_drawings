...
import { ct_tab_BaseunitFridgeMapping, ICT_tab_BaseunitFridgeMapping, cto_tab_BaseunitFridgeMapping, ct2_tab_BaseunitFridgeMapping } from '../custom-tables/tab_BaseunitFridgeMapping'
...
import { ct_tab_CornerunitStraightDimensionLogic, ICT_tab_CornerunitStraightDimensionLogic, cto_tab_CornerunitStraightDimensionLogic, ct2_tab_CornerunitStraightDimensionLogic } from '../custom-tables/tab_CornerunitStraightDimensionLogic'
...
import { ct_tab_FrameFillingMapping, ICT_tab_FrameFillingMapping, cto_tab_FrameFillingMapping, ct2_tab_FrameFillingMapping } from '../custom-tables/tab_FrameFillingMapping'
...
import { ct_tab_FridgeNicheConstruction, ICT_tab_FridgeNicheConstruction, cto_tab_FridgeNicheConstruction, ct2_tab_FridgeNicheConstruction } from '../custom-tables/tab_FridgeNicheConstruction'
...
import { ct_tab_HoodAssemblyMapping, ICT_tab_HoodAssemblyMapping, cto_tab_HoodAssemblyMapping, ct2_tab_HoodAssemblyMapping } from '../custom-tables/tab_HoodAssemblyMapping'
import { ct_tab_HoodAssemblyParts, ICT_tab_HoodAssemblyParts, cto_tab_HoodAssemblyParts, ct2_tab_HoodAssemblyParts } from '../custom-tables/tab_HoodAssemblyParts'
import { ct_tab_HoodConstruction, ICT_tab_HoodConstruction, cto_tab_HoodConstruction, ct2_tab_HoodConstruction } from '../custom-tables/tab_HoodConstruction'
import { ct_tab_HoodMapping, ICT_tab_HoodMapping, cto_tab_HoodMapping, ct2_tab_HoodMapping } from '../custom-tables/tab_HoodMapping'
import { ct_tab_LaundryMachineMapping, ICT_tab_LaundryMachineMapping, cto_tab_LaundryMachineMapping, ct2_tab_LaundryMachineMapping } from '../custom-tables/tab_LaundryMachineMapping'
...
import { ct_tab_PantryPulloutColorMapping, ICT_tab_PantryPulloutColorMapping, cto_tab_PantryPulloutColorMapping, ct2_tab_PantryPulloutColorMapping } from '../custom-tables/tab_PantryPulloutColorMapping'
import { ct_tab_PantryPulloutConstruction, ICT_tab_PantryPulloutConstruction, cto_tab_PantryPulloutConstruction, ct2_tab_PantryPulloutConstruction } from '../custom-tables/tab_PantryPulloutConstruction'
import { ct_tab_PantryPulloutMapping, ICT_tab_PantryPulloutMapping, cto_tab_PantryPulloutMapping, ct2_tab_PantryPulloutMapping } from '../custom-tables/tab_PantryPulloutMapping'
...
import { ct_tab_PlinthAreaBaseboardSettings, ICT_tab_PlinthAreaBaseboardSettings, cto_tab_PlinthAreaBaseboardSettings, ct2_tab_PlinthAreaBaseboardSettings } from '../custom-tables/tab_PlinthAreaBaseboardSettings'
...
import { ct_tab_SegmentFrontConstruction, ICT_tab_SegmentFrontConstruction, cto_tab_SegmentFrontConstruction, ct2_tab_SegmentFrontConstruction } from '../custom-tables/tab_SegmentFrontConstruction'
import { ct_tab_SegmentFrontMapping, ICT_tab_SegmentFrontMapping, cto_tab_SegmentFrontMapping, ct2_tab_SegmentFrontMapping } from '../custom-tables/tab_SegmentFrontMapping'
...
export interface IP_part_BaseunitFridgePanelWoodFrame_PartVarsReadOnly extends IPartBase {
  ...
  get pa_FrameFillingColor(): string;
...
export interface IP_part_BaseunitFridgePanelWoodFrame_PartVarsWritable extends IP_part_BaseunitFridgePanelWoodFrame_PartVarsReadOnly {
  ...
  set pa_FrameFillingColor(value: string);
...
export class P_part_BaseunitFridgePanelWoodFrame extends PartBase implements IP_part_BaseunitFridgePanelWoodFrame_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_PanelWoodFrame01,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_BaseunitFridgePanelWoodFrame', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  override getAttributes(): Map<string, any> {
    ...
    res.set('pa_FrameFillingColor', this.#pa_FrameFillingColor);
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_BaseunitFridgePanelWoodFrame_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_FrameFillingColor === undefined) {
      logError('Attribute pa_FrameFillingColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_FrameFillingMaterial === undefined) {
      logError('Attribute pa_FrameFillingMaterial not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_FrameFillingThk === undefined) {
      logError('Attribute pa_FrameFillingThk not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_FramePartsWidthHor === undefined) {
      logError('Attribute pa_FramePartsWidthHor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_FramePartsWidthVert === undefined) {
      logError('Attribute pa_FramePartsWidthVert not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_PanelWoodFrameType === undefined) {
      logError('Attribute pa_PanelWoodFrameType not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_TypeElement === undefined) {
      logError('Attribute pa_TypeElement not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_Weight === undefined) {
      logError('Attribute pa_Weight not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
    if (this.#pa_DoorDirection === undefined) {
      logError('Attribute pa_DoorDirection not set in part part_BaseunitFridgePanelWoodFrame (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM225 extends BOM_ElementType_bomout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_BaseunitFridgePanelWoodFrame_PartVarsReadOnly) {
            const part = partSelf;
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
    try {
      // ncout_Group: 638: part_HandleDrill FromFront
      if (touch == PartTouch.FromFront && partNext instanceof P_part_HandleDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          ...
          const _bom = new class BOMT638 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_BaseunitFridgePanelWoodFrame_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HandleDrill_PartVarsReadOnly, posRel: CollisionDataRel) {
  ...
  override createBomTouchElementsEnd(): void {
    try {
      ...
      if (true) { // ####################### CUSTOM SCRIPTS ########################
        let parentId = partSelf._id;
        const _bom = new class BOMT639 extends BOM_ElementType_ncout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_BaseunitFridgePanelWoodFrame_PartVarsReadOnly) {
  ...
  get pa_FrameFillingColor(): string { return this.#pa_FrameFillingColor!; }
  set pa_FrameFillingColor(val: string) { this.#pa_FrameFillingColor = val; }
  #pa_FrameFillingColor?: string;
