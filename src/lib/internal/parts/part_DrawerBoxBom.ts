...
export class P_part_DrawerBoxBom extends PartBase implements IP_part_DrawerBoxBom_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_DrawerBox01,
    ...
    super(parent, 'part_DrawerBoxBom', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  ...
  checkAttributes() {
    ...
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_DrawerBoxBom (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_DrawerBoxBom (id: ' + this._id + ')');
    }
