...
export interface IP_part_PulloutHardwareDrillings_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_PulloutHardware01;
  get pa_ProcessingId(): string;
  get pa_ProcessingId2(): string;
}

export interface IP_part_PulloutHardwareDrillings_PartVarsWritable extends IP_part_PulloutHardwareDrillings_PartVarsReadOnly {
  set pa_ProcessingId(value: string);
  set pa_ProcessingId2(value: string);
}

export class P_part_PulloutHardwareDrillings extends PartBase implements IP_part_PulloutHardwareDrillings_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_PulloutHardware01,
    ...
    super(parent, 'part_PulloutHardwareDrillings', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._hidden = true;
  }
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_PulloutHardwareDrillings_ValueScript
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_PulloutHardwareDrillings (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId2 === undefined) {
      logError('Attribute pa_ProcessingId2 not set in part part_PulloutHardwareDrillings (id: ' + this._id + ')');
    }
