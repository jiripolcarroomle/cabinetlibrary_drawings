...
export class P_part_Filler extends PartBase implements IP_part_Filler_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FrontPanel01,
    ...
    super(parent, 'part_Filler', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_Filler (id: ' + this._id + ')');
    }
  ...
  override createBomTouchElementsStart(): void {
    try {
      ...
      if (partSelf.pa_ProcessingId != '') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOMT656 extends BOM_ElementType_ncout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_Filler_PartVarsReadOnly) {
