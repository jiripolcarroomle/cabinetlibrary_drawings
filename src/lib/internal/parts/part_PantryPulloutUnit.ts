...
import { IPartParentsNonNull_mc_PantryPullout01 } from '../mod-interfaces'

export interface IP_part_PantryPulloutUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_PantryPullout01;
  get pa_BomId(): string;
  get pa_PartgroupBomId(): string;
}

export interface IP_part_PantryPulloutUnit_PartVarsWritable extends IP_part_PantryPulloutUnit_PartVarsReadOnly {
  set pa_BomId(value: string);
  set pa_PartgroupBomId(value: string);
}

export class P_part_PantryPulloutUnit extends PartBase implements IP_part_PantryPulloutUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_PantryPullout01,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_PantryPulloutUnit', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_PantryPullout01; // module which created this part
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_PantryPulloutUnit_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_PantryPulloutUnit (id: ' + this._id + ')');
    }
    if (this.#pa_PartgroupBomId === undefined) {
      logError('Attribute pa_PartgroupBomId not set in part part_PantryPulloutUnit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      const partSelf = this;
      // bomout_PartGroup: 230
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM230 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_PantryPulloutUnit_PartVarsReadOnly) {
            const part = partSelf;
            ...
            try {
              ...
              let name = 'Partgroup pantry pullout'
        ...
        _bom._description = "Partgroup for pantry pullout";
        _bom.set(partSelf);
