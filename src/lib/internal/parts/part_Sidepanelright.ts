...
export class P_part_Sidepanelright extends PartBase implements IP_part_Sidepanelright_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_StorageunitSidepanel01,
    ...
    super(parent, 'part_Sidepanelright', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'YZX';
  }
  ...
  setAttributes() {
    try {
      ...
      if (this.#pa_EdgeFrontColor === undefined) {
        this.pa_EdgeFrontColor = this.parent.mod_CarcaseVisRight
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeFrontColor;;
      }
      if (this.#pa_EdgeLeftColor === undefined) {
        this.pa_EdgeLeftColor = (this.parent.mod_CarcaseVisRight || this.parent.mod_CarcaseVisBtm)
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeColor;;
      }
      if (this.#pa_EdgeBackColor === undefined) {
        this.pa_EdgeBackColor = this.parent.mod_CarcaseVisRight
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeBackColor;;
      }
      if (this.#pa_EdgeRightColor === undefined) {
        this.pa_EdgeRightColor = (this.parent.mod_CarcaseVisRight || this.parent.mod_CarcaseVisTop)
          ? this.parent.mod_CarcaseOutsideColor
          : this.parent.mod_CarcaseEdgeColor;;
      }
  ...
  createBomElements() {
    try {
      ...
      if (partSelf._g?.basic_CreateBom != null && partSelf._g.basic_CreateBom !== 'Custom') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM239 extends BOM_ElementType_bomout_Group {
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
    ...
    try {
      // ncout_Group: 641: part_PulloutHardwareDrillings FromLeft
      if (touch == PartTouch.FromLeft && partNext instanceof P_part_PulloutHardwareDrillings) {
        let part2 = <P_part_PulloutHardwareDrillings>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT641 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Sidepanelright_PartVarsReadOnly, touch: PartTouch, part2: IP_part_PulloutHardwareDrillings_PartVarsReadOnly, posRel: CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
              try {
                ...
                let drillings: ICT_tab_HardwareDrillVertLibrary[] = [];
                const drillID = part2.pa_ProcessingId;
                let processings = GlobalFunc.find_ProcessingMapping(drillID);
          ...
          _bom._parentId = partSelf._id;
          _bom._description = "Drillings for Pullout Hardware";
          _bom.set(partSelf, touch, part2, posRel);
          partSelf._bom.push(_bom);
          if (_bom._subBom) {
            _bom._subBom.forEach(p => partSelf._bom.push(p));
    ...
    try {
      // ncout_Group: 666: part_ClothingOrganizerDrilling FromLeft
      if (touch == PartTouch.FromLeft && partNext instanceof P_part_ClothingOrganizerDrilling) {
        let part2 = <P_part_ClothingOrganizerDrilling>partNext;
        const partSelf = this;
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT666 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_Sidepanelright_PartVarsReadOnly, touch: PartTouch, part2: IP_part_ClothingOrganizerDrilling_PartVarsReadOnly, posRel: CollisionDataRel) {
              this._touchOrigin = touch;
              this._touchPart2Id = part2._id;
              this._touchPart2Name = part2._partId;
              internal_enterBomPartMasterDataTouches(partSelf._id, partSelf._partId, part2._id, part2._partId, touch);
              try {
                ...
                processings.forEach((processing) => {
                  if (processing.ProcessingLibrary == 'DrillVertical') {
                    let foundDrillings = GlobalFunc.find_HardwareDrillVertLibrary(processing.ProcessingId!, 'Side');
                    drillings = drillings.concat(foundDrillings);
                  }
                });
          ...
          _bom._parentId = partSelf._id;
          _bom._description = "Connection with the clothing organizer";
          _bom.set(partSelf, touch, part2, posRel);
          partSelf._bom.push(_bom);
          if (_bom._subBom) {
            _bom._subBom.forEach(p => partSelf._bom.push(p));
