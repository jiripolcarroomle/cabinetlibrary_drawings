...
export class P_part_Heatshelf extends PartBase implements IP_part_Heatshelf_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_StorageunitShelftop02,
    ...
    super(parent, 'part_Heatshelf', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XZY';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_ColorGrainGroup === undefined) {
      logError('Attribute pa_ColorGrainGroup not set in part part_Heatshelf (id: ' + this._id + ')');
    }
    if (this.#pa_ProgramGrainGroup === undefined) {
      logError('Attribute pa_ProgramGrainGroup not set in part part_Heatshelf (id: ' + this._id + ')');
    }
