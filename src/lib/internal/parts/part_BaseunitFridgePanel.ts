...
export interface IP_part_BaseunitFridgePanel_PartVarsReadOnly extends IPartBase {
...
export interface IP_part_BaseunitFridgePanel_PartVarsWritable extends IP_part_BaseunitFridgePanel_PartVarsReadOnly {
...
export class P_part_BaseunitFridgePanel extends PartBase implements IP_part_BaseunitFridgePanel_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_FrontPanel01,
    ...
    super(parent, 'part_BaseunitFridgePanel', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_BaseunitFridgePanel_ValueScript
  ...
  checkAttributes() {
    ...
    if (this.#pa_AdditionalInfo1 === undefined) {
      logError('Attribute pa_AdditionalInfo1 not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_BtmColor === undefined) {
      logError('Attribute pa_BtmColor not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackColor === undefined) {
      logError('Attribute pa_EdgeBackColor not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeBackType === undefined) {
      logError('Attribute pa_EdgeBackType not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontColor === undefined) {
      logError('Attribute pa_EdgeFrontColor not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeFrontType === undefined) {
      logError('Attribute pa_EdgeFrontType not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeJointType === undefined) {
      logError('Attribute pa_EdgeJointType not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftColor === undefined) {
      logError('Attribute pa_EdgeLeftColor not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeLeftType === undefined) {
      logError('Attribute pa_EdgeLeftType not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightColor === undefined) {
      logError('Attribute pa_EdgeRightColor not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_EdgeRightType === undefined) {
      logError('Attribute pa_EdgeRightType not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_GrainDirection === undefined) {
      logError('Attribute pa_GrainDirection not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_HardwareInfo === undefined) {
      logError('Attribute pa_HardwareInfo not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_Program === undefined) {
      logError('Attribute pa_Program not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_TopColor === undefined) {
      logError('Attribute pa_TopColor not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_Weight === undefined) {
      logError('Attribute pa_Weight not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
    if (this.#pa_ProcessingId === undefined) {
      logError('Attribute pa_ProcessingId not set in part part_BaseunitFridgePanel (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      ...
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM224 extends BOM_ElementType_bomout_Board {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_BaseunitFridgePanel_PartVarsReadOnly) {
            const part = partSelf;
  ...
  override createBomTouchElementsStart(): void {
    try {
      const partSelf = this;
      if (partSelf.pa_ProcessingId != '') { // ####################### CUSTOM SCRIPTS ########################
        let parentId = partSelf._id;
        const _bom = new class BOMT662 extends BOM_ElementType_ncout_Group {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_BaseunitFridgePanel_PartVarsReadOnly) {
            internal_enterBomPartMasterDataTouchesStart(partSelf._id, partSelf._partId);
            try {
              // ####################### CUSTOM SCRIPTS ########################
        ...
        _bom._description = "Multiple Processings";
        _bom.set(partSelf);
        partSelf._bom.push(_bom);
        if (_bom._subBom) {
  ...
  override createBomTouchElements(touch: PartTouch, partNext: PartBase, posRel: CollisionDataRel) {
    try {
      // ncout_Group: 637: part_HandleDrill FromFront
      if (touch == PartTouch.FromFront && partNext instanceof P_part_HandleDrill) {
        ...
        if (true) { // ####################### CUSTOM SCRIPTS ########################
          let parentId = partSelf._id;
          const _bom = new class BOMT637 extends BOM_ElementType_ncout_Group {
            constructor() { super(partSelf); }
            set(partSelf: IP_part_BaseunitFridgePanel_PartVarsReadOnly, touch: PartTouch, part2: IP_part_HandleDrill_PartVarsReadOnly, posRel: CollisionDataRel) {
