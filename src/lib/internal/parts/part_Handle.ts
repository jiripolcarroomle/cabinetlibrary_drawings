...
export class P_part_Handle extends PartBase implements IP_part_Handle_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Handle01,
    ...
    super(parent, 'part_Handle', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
  }
  ...
  checkAttributes() {
    ...
    if (this.#pa_ParentName === undefined) {
      logError('Attribute pa_ParentName not set in part part_Handle (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      const partSelf = this;
      // bomout_Group: 152
      if (//partSelf._g?.basic_CreateBom == 'Library'
        1 == 1) { // ####################### CUSTOM SCRIPTS ########################
        ...
        const _bom = new class BOM152 extends BOM_ElementType_bomout_Group {
          ...
          set(partSelf: IP_part_Handle_PartVarsReadOnly) {
            ...
            try {
              ...
              GlobalFunc.process_HardwareBom(this, part, elementType, elementCategory, parentBomId, part.pa_HardwareId)
              // ################### END CUSTOM SCRIPTS ########################
            }
            finally {
