...
import { IPartParentsNonNull_mc_Pullout01 } from '../mod-interfaces'

export interface IP_part_PulloutUnit_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mc_Pullout01;
  get pa_BomId(): string;
  get pa_PartgroupBomId(): string;
}

export interface IP_part_PulloutUnit_PartVarsWritable extends IP_part_PulloutUnit_PartVarsReadOnly {
  set pa_BomId(value: string);
  set pa_PartgroupBomId(value: string);
}

export class P_part_PulloutUnit extends PartBase implements IP_part_PulloutUnit_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mc_Pullout01,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_PulloutUnit', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mc_Pullout01; // module which created this part
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_PulloutUnit (id: ' + this._id + ')');
    }
    if (this.#pa_PartgroupBomId === undefined) {
      logError('Attribute pa_PartgroupBomId not set in part part_PulloutUnit (id: ' + this._id + ')');
    }
  ...
  createBomElements() {
    try {
      const partSelf = this;
      // bomout_PartGroup: 229
      if (partSelf._g?.basic_CreateBom == 'Library') { // ####################### CUSTOM SCRIPTS ########################
        const parentId = partSelf._id;
        const _bom = new class BOM229 extends BOM_ElementType_bomout_PartGroup {
          constructor() { super(partSelf); }
          set(partSelf: IP_part_PulloutUnit_PartVarsReadOnly) {
            const part = partSelf;
        ...
        _bom._description = "Partgroup for pullout";
        _bom.set(partSelf);
