...
import { IPartParentsNonNull_mr_PlinthAreaBaseboard } from '../mod-interfaces'

export interface IP_part_BaseboardGroup_PartVarsReadOnly extends IPartBase {
  parent: IPartParentsNonNull_mr_PlinthAreaBaseboard;
  get pa_BomId(): string;
}

export interface IP_part_BaseboardGroup_PartVarsWritable extends IP_part_BaseboardGroup_PartVarsReadOnly {
  set pa_BomId(value: string);
}

export class P_part_BaseboardGroup extends PartBase implements IP_part_BaseboardGroup_PartVarsWritable {
  constructor(parent: IPartParentsNonNull_mr_PlinthAreaBaseboard,
    x: number, y: number, z: number, dimx: number, dimy: number, dimz: number, isContainerPart?: boolean) {
    super(parent, 'part_BaseboardGroup', x, y, z, dimx, dimy, dimz, isContainerPart);
    this.parent = parent;
    this._wdt = 'XYZ';
    this._noNCData = true;
    this._hidden = true;
  }
  parent: IPartParentsNonNull_mr_PlinthAreaBaseboard; // module which created this part
  ...
  setAttributes() {
    try {
      ...
      // ####################### CUSTOM SCRIPTS ########################
      // ###############################################################
      // part_BaseboardGroup_ValueScript
      // ###############################################################
      // ################### END CUSTOM SCRIPTS ########################
      // ###############################################################
    }
  ...
  checkAttributes() {
    ...
    if (this.#pa_BomId === undefined) {
      logError('Attribute pa_BomId not set in part part_BaseboardGroup (id: ' + this._id + ')');
    }
