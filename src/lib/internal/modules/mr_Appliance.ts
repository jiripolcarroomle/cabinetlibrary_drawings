...
export interface adc_mr_Appliance extends adc_base_mr_Appliance {
  addOD_M_mf_Dishwasher(index?: number): dc_mf_Dishwasher;
  addOD_M_mf_BaseunitFridge(index?: number): dc_mf_BaseunitFridge;
}

export interface ccm_mr_Appliance extends adc_base_mr_Appliance {
}
...
export class OD_M_mr_Appliance extends OD_Base implements dc_mr_Appliance
  ...
  #dataCompletionSetDefaultValues() {
    ...
    if (this.mod_FrameFillingType === undefined) {
      this.mod_FrameFillingType = "Automatic";
    }
  ...
  addOD_M_mf_BaseunitFridge(index?: number): dc_mf_BaseunitFridge {
    var m = new OD_M_mf_BaseunitFridge(this);
    m.dataCompletion();
    if (this._manufacturerMode) {
      m.manufacturerDataCompletion();
    }
  ...
  override validateSelections(): void {
    ...
    this.mod_Model_matrix = VariantValidation.mod_Model(this, this.mod_Model);
  ...
  override calculateContainerModules(): void {
    let ccm = new OD_M_mr_Appliance_NonNull(this);
  ...
  addpart_PlinthAreaUnit(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_PlinthAreaUnit_PartVarsWritable {
    var c = new P_part_PlinthAreaUnit(this._selfNonNull, x, y, z, dimx, dimy, dimz, undefined);
    this.p.push(c);
    return c;
  }
...
class OD_M_mr_Appliance_NonNull implements cbp_mr_Appliance, adc_mr_Appliance, ccm_mr_Appliance, IPartParentsNonNull_mr_Appliance_mr_Filler01, IPartParentsNonNull_mc_Dishwasher01_mr_Appliance, IPartParentsNonNull_mr_Appliance {
  ...
  #calculateContainerModulesInternal = mr_Appliance_calculateContainerModules;
  ...
  addOD_M_mf_BaseunitFridge(index?: number): dc_mf_BaseunitFridge {
    return this.#internalParent.addOD_M_mf_BaseunitFridge(index);
  }
}
