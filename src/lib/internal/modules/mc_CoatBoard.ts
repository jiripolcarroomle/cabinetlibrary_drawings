...
import { IModParents_mr_CoatBoard } from '../mod-interfaces'
import { IP_part_CoatBoard_PartVarsWritable, P_part_CoatBoard } from '../parts/part_CoatBoard'
import { IP_part_CoatBoardGroup_PartVarsWritable, P_part_CoatBoardGroup } from '../parts/part_CoatBoardGroup'
import { IModuleNonNull_mc_CoatBoard, IModParents_mc_CoatBoard_mr_CoatBoard, IModParents_mc_CoatBoard, IPartParentsNonNull_mc_CoatBoard_mr_CoatBoard, IPartParentsNonNull_mc_CoatBoard } from '../mod-interfaces'
import { IModVar_mod_CoatBoardColor, IModVarNonNull_mod_CoatBoardColor, IModVar_mod_CoatBoardProgram, IModVarNonNull_mod_CoatBoardProgram, IModVar_mod_CoatBoardId, IModVarNonNull_mod_CoatBoardId, IModVar_mod_Width, IModVarNonNull_mod_Width, IModVar_mod_Height, IModVarNonNull_mod_Height, IModVar_mod_CoatBoardThickness, IModVarNonNull_mod_CoatBoardThickness } from '../var-interfaces'
import { VariantValidation, IMatrix_mod_CoatBoardColor, IMatrix_mod_CoatBoardProgram, IMatrix_mod_CoatBoardId, IMatrix_mod_Width, IMatrix_mod_Height, IMatrix_mod_CoatBoardThickness } from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_CoatBoard, IPartAdd_part_CoatBoardGroup } from '../part-add-interfaces';
import { mc_CoatBoard_createBuildPlan, mc_CoatBoard_afterDataCompletion, mc_CoatBoard_manufacturerDataCompletion, mc_CoatBoard_calculateContainerModules, mc_CoatBoard_prepareContext } from '../../modules/mc_CoatBoard';

export interface cbp_mc_CoatBoard extends IModBaseProp
  , IPartAdd_part_CoatBoard, IPartAdd_part_CoatBoardGroup, IModVarNonNull_mod_CoatBoardColor, IModVarNonNull_mod_CoatBoardProgram, IModVarNonNull_mod_CoatBoardId, IModVarNonNull_mod_Width, IModVarNonNull_mod_Height, IModVarNonNull_mod_CoatBoardThickness {
  parent: IModParents_mr_CoatBoard;
...
export interface dc_mc_CoatBoard extends IModBaseProp
  , IModVar_mod_CoatBoardColor, IModVar_mod_CoatBoardProgram, IModVar_mod_CoatBoardId, IModVar_mod_Width, IModVar_mod_Height, IModVar_mod_CoatBoardThickness {
  m: OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
  seal(): IModuleNonNull_mc_CoatBoard;
  parent: IModParents_mr_CoatBoard;
}

export interface pc_mc_CoatBoard extends dc_mc_CoatBoard {
...
export interface adc_base_mc_CoatBoard extends IModBaseProp
  , IModVarNonNull_mod_CoatBoardColor, IModVarNonNull_mod_CoatBoardProgram, IModVarNonNull_mod_CoatBoardId, IModVarNonNull_mod_Width, IModVarNonNull_mod_Height, IModVarNonNull_mod_CoatBoardThickness {
  get m(): OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
}

export interface adc_mc_CoatBoard extends adc_base_mc_CoatBoard {
}

export interface ccm_mc_CoatBoard extends adc_base_mc_CoatBoard {
}


export class OD_M_mc_CoatBoard extends OD_Base implements pc_mc_CoatBoard, dc_mc_CoatBoard
  , IModParents_mc_CoatBoard_mr_CoatBoard, IModParents_mc_CoatBoard
  , IModVar_mod_CoatBoardColor, IModVar_mod_CoatBoardProgram, IModVar_mod_CoatBoardId, IModVar_mod_Width, IModVar_mod_Height, IModVar_mod_CoatBoardThickness {
  constructor(parent: IModParents_mr_CoatBoard, manufacturerMode?: boolean) {
    super('mc_CoatBoard', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_CoatBoard_NonNull(this);
  }
  parent: IModParents_mr_CoatBoard;
  _selfNonNull: OD_M_mc_CoatBoard_NonNull;

  override variants: string[] = ['mod_CoatBoardColor', 'mod_CoatBoardProgram', 'mod_CoatBoardId', 'mod_Width', 'mod_Height', 'mod_CoatBoardThickness',];
  ...
  #dataCompletionAssignDerivedData() {
    ...
    if (this.mod_CoatBoardColor === undefined) {
      ...
      while (s !== undefined) {
        if (s.variants.includes('mod_CoatBoardColor')) {
          let pv = <IModVar_mod_CoatBoardColor>s;
          if (pv.mod_CoatBoardColor !== undefined) {
            this.mod_CoatBoardColor = (<IModVar_mod_CoatBoardColor>s).mod_CoatBoardColor;
            break;
          };
        }
        s = s.parentBase;
    ...
    if (this.mod_CoatBoardProgram === undefined) {
      ...
      // try to find a parent with a valid value
      while (s !== undefined) {
        if (s.variants.includes('mod_CoatBoardProgram')) {
          let pv = <IModVar_mod_CoatBoardProgram>s;
          if (pv.mod_CoatBoardProgram !== undefined) {
            this.mod_CoatBoardProgram = (<IModVar_mod_CoatBoardProgram>s).mod_CoatBoardProgram;
            break;
          };
        }
        s = s.parentBase;
    ...
    if (this.mod_CoatBoardId === undefined) {
      ...
      // try to find a parent with a valid value
      while (s !== undefined) {
        if (s.variants.includes('mod_CoatBoardId')) {
          let pv = <IModVar_mod_CoatBoardId>s;
          if (pv.mod_CoatBoardId !== undefined) {
            this.mod_CoatBoardId = (<IModVar_mod_CoatBoardId>s).mod_CoatBoardId;
            break;
          };
        }
        s = s.parentBase;
    ...
    if (this.mod_CoatBoardThickness === undefined) {
      ...
      // try to find a parent with a valid value
      while (s !== undefined) {
        if (s.variants.includes('mod_CoatBoardThickness')) {
          let pv = <IModVar_mod_CoatBoardThickness>s;
          if (pv.mod_CoatBoardThickness !== undefined) {
            this.mod_CoatBoardThickness = (<IModVar_mod_CoatBoardThickness>s).mod_CoatBoardThickness;
            break;
          };
        }
        s = s.parentBase;
  ...
  #manufacturerDataCompletionInternal = mc_CoatBoard_manufacturerDataCompletion;
  override afterDataCompletion(): void {
    if (this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_CoatBoard_NonNull(this);
  ...
  #prepareContextInternal = mc_CoatBoard_prepareContext;
  seal(): IModuleNonNull_mc_CoatBoard {
    this.afterDataCompletion();
    const adc = new OD_M_mc_CoatBoard_NonNull(this);
    return adc;
  }
  ...
  override calculateContainerModules(): void {
    let ccm = new OD_M_mc_CoatBoard_NonNull(this);
  ...
  addpart_CoatBoard(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_CoatBoard_PartVarsWritable {
    var c = new P_part_CoatBoard(this._selfNonNull, x, y, z, dimx, dimy, dimz, undefined);
    this.p.push(c);
    return c;
  }
  ...
  override createBuildPlan(): void {
    var cbp = new OD_M_mc_CoatBoard_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_CoatBoard_NonNull implements cbp_mc_CoatBoard, adc_mc_CoatBoard, ccm_mc_CoatBoard, IPartParentsNonNull_mc_CoatBoard_mr_CoatBoard, IPartParentsNonNull_mc_CoatBoard {
  constructor(parent: OD_M_mc_CoatBoard) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_CoatBoard;
  ...
  #createBuildPlanInternal = mc_CoatBoard_createBuildPlan;
  afterDataCompletion(): void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_CoatBoard_afterDataCompletion;
  calculateContainerModules(): void {
    this.#calculateContainerModulesInternal();
  }
  #calculateContainerModulesInternal = mc_CoatBoard_calculateContainerModules;
  ...
  parent: IModParents_mr_CoatBoard;
  addpart_CoatBoard(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_CoatBoard_PartVarsWritable {
    return this.#internalParent.addpart_CoatBoard(x, y, z, dimx, dimy, dimz);
  }
  addpart_CoatBoardGroup(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_CoatBoardGroup_PartVarsWritable {
