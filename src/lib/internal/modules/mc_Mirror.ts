...
import { IModParents_mc_MirrorBoard } from '../mod-interfaces'
import { IP_part_Mirror_PartVarsWritable, P_part_Mirror } from '../parts/part_Mirror'
import { IModuleNonNull_mc_Mirror, IModParents_mc_Mirror, IPartParentsNonNull_mc_Mirror } from '../mod-interfaces'
import { IModVar_mod_FrontId, IModVarNonNull_mod_FrontId, IModVar_mod_MirrorColor, IModVarNonNull_mod_MirrorColor, IModVar_mod_MirrorWidth, IModVarNonNull_mod_MirrorWidth, IModVar_mod_MirrorHeight, IModVarNonNull_mod_MirrorHeight, IModVar_mod_MirrorThickness, IModVarNonNull_mod_MirrorThickness } from '../var-interfaces'
import { VariantValidation, IMatrix_mod_FrontId, IMatrix_mod_MirrorColor, IMatrix_mod_MirrorWidth, IMatrix_mod_MirrorHeight, IMatrix_mod_MirrorThickness } from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_Mirror } from '../part-add-interfaces';
import { mc_Mirror_createBuildPlan, mc_Mirror_afterDataCompletion, mc_Mirror_manufacturerDataCompletion, mc_Mirror_calculateContainerModules, mc_Mirror_prepareContext } from '../../modules/mc_Mirror';

export interface cbp_mc_Mirror extends IModBaseProp
  , IPartAdd_part_Mirror, IModVarNonNull_mod_FrontId, IModVarNonNull_mod_MirrorColor, IModVarNonNull_mod_MirrorWidth, IModVarNonNull_mod_MirrorHeight, IModVarNonNull_mod_MirrorThickness {
  parent: IModParents_mc_MirrorBoard;
...
export interface dc_mc_Mirror extends IModBaseProp
  , IModVar_mod_FrontId, IModVar_mod_MirrorColor, IModVar_mod_MirrorWidth, IModVar_mod_MirrorHeight, IModVar_mod_MirrorThickness {
  m: OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
  seal(): IModuleNonNull_mc_Mirror;
  parent: IModParents_mc_MirrorBoard;
}

export interface pc_mc_Mirror extends dc_mc_Mirror {
...
export interface adc_base_mc_Mirror extends IModBaseProp
  , IModVarNonNull_mod_FrontId, IModVarNonNull_mod_MirrorColor, IModVarNonNull_mod_MirrorWidth, IModVarNonNull_mod_MirrorHeight, IModVarNonNull_mod_MirrorThickness {
  get m(): OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
}

export interface adc_mc_Mirror extends adc_base_mc_Mirror {
}

export interface ccm_mc_Mirror extends adc_base_mc_Mirror {
}


export class OD_M_mc_Mirror extends OD_Base implements pc_mc_Mirror, dc_mc_Mirror
  , IModParents_mc_Mirror
  , IModVar_mod_FrontId, IModVar_mod_MirrorColor, IModVar_mod_MirrorWidth, IModVar_mod_MirrorHeight, IModVar_mod_MirrorThickness {
  constructor(parent: IModParents_mc_MirrorBoard, manufacturerMode?: boolean) {
    super('mc_Mirror', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_Mirror_NonNull(this);
  }
  parent: IModParents_mc_MirrorBoard;
  _selfNonNull: OD_M_mc_Mirror_NonNull;

  override variants: string[] = ['mod_FrontId', 'mod_MirrorColor', 'mod_MirrorWidth', 'mod_MirrorHeight', 'mod_MirrorThickness',];
  #mod_FrontId?: string;
  ...
  #manufacturerDataCompletionInternal = mc_Mirror_manufacturerDataCompletion;
  override afterDataCompletion(): void {
    if (this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_Mirror_NonNull(this);
  ...
  #prepareContextInternal = mc_Mirror_prepareContext;
  seal(): IModuleNonNull_mc_Mirror {
    this.afterDataCompletion();
    const adc = new OD_M_mc_Mirror_NonNull(this);
    return adc;
  }
  ...
  override calculateContainerModules(): void {
    let ccm = new OD_M_mc_Mirror_NonNull(this);
    ccm.calculateContainerModules();
    this.m.forEach(subMod => subMod.calculateContainerModules());
  }

  addpart_Mirror(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_Mirror_PartVarsWritable {
    var c = new P_part_Mirror(this._selfNonNull, x, y, z, dimx, dimy, dimz, undefined);
    this.p.push(c);
    return c;
  }

  override createBuildPlan(): void {
    var cbp = new OD_M_mc_Mirror_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_Mirror_NonNull implements cbp_mc_Mirror, adc_mc_Mirror, ccm_mc_Mirror, IPartParentsNonNull_mc_Mirror {
  constructor(parent: OD_M_mc_Mirror) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_Mirror;
  ...
  #createBuildPlanInternal = mc_Mirror_createBuildPlan;
  afterDataCompletion(): void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_Mirror_afterDataCompletion;
  calculateContainerModules(): void {
    this.#calculateContainerModulesInternal();
  }
  #calculateContainerModulesInternal = mc_Mirror_calculateContainerModules;
  ...
  parent: IModParents_mc_MirrorBoard;
  addpart_Mirror(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_Mirror_PartVarsWritable {
    return this.#internalParent.addpart_Mirror(x, y, z, dimx, dimy, dimz);
  }
