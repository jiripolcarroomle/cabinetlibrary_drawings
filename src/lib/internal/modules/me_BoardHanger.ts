...
import { OD_M_mc_BoardHanger, dc_mc_BoardHanger } from './mc_BoardHanger'
import { IModuleNonNull_me_BoardHanger, IModParents_me_BoardHanger, IPartParentsNonNull_me_BoardHanger } from '../mod-interfaces'
import { VariantValidation } from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { me_BoardHanger_createBuildPlan, me_BoardHanger_afterDataCompletion, me_BoardHanger_manufacturerDataCompletion, me_BoardHanger_calculateContainerModules, me_BoardHanger_prepareContext } from '../../modules/me_BoardHanger';

export interface cbp_me_BoardHanger extends IModBaseProp {
...
export interface dc_me_BoardHanger extends IModBaseProp {
  m: OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
  seal(): IModuleNonNull_me_BoardHanger;
  parent: IModParents_mf_BoardShelf;
  addOD_M_mc_BoardHanger(index?: number): dc_mc_BoardHanger;
}

export interface pc_me_BoardHanger extends dc_me_BoardHanger {
...
export interface adc_base_me_BoardHanger extends IModBaseProp {
  get m(): OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
}

export interface adc_me_BoardHanger extends adc_base_me_BoardHanger {
  addOD_M_mc_BoardHanger(index?: number): dc_mc_BoardHanger;
}

export interface ccm_me_BoardHanger extends adc_base_me_BoardHanger {
}


export class OD_M_me_BoardHanger extends OD_Base implements pc_me_BoardHanger, dc_me_BoardHanger
  , IModParents_me_BoardHanger {
  constructor(parent: IModParents_mf_BoardShelf, manufacturerMode?: boolean) {
    super('me_BoardHanger', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_me_BoardHanger_NonNull(this);
  }
  parent: IModParents_mf_BoardShelf;
  _selfNonNull: OD_M_me_BoardHanger_NonNull;
  ...
  #manufacturerDataCompletionInternal = me_BoardHanger_manufacturerDataCompletion;
  override afterDataCompletion(): void {
    if (this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_me_BoardHanger_NonNull(this);
  ...
  #prepareContextInternal = me_BoardHanger_prepareContext;
  seal(): IModuleNonNull_me_BoardHanger {
    this.afterDataCompletion();
    const adc = new OD_M_me_BoardHanger_NonNull(this);
    return adc;
  }

  addOD_M_mc_BoardHanger(index?: number): dc_mc_BoardHanger {
    var m = new OD_M_mc_BoardHanger(this);
    m.dataCompletion();
  ...
  override calculateContainerModules(): void {
    let ccm = new OD_M_me_BoardHanger_NonNull(this);
  ...
  override createBuildPlan(): void {
    var cbp = new OD_M_me_BoardHanger_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_me_BoardHanger_NonNull implements cbp_me_BoardHanger, adc_me_BoardHanger, ccm_me_BoardHanger, IPartParentsNonNull_me_BoardHanger {
  constructor(parent: OD_M_me_BoardHanger) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_me_BoardHanger;
  ...
  #createBuildPlanInternal = me_BoardHanger_createBuildPlan;
  afterDataCompletion(): void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = me_BoardHanger_afterDataCompletion;
  calculateContainerModules(): void {
    this.#calculateContainerModulesInternal();
  }
  #calculateContainerModulesInternal = me_BoardHanger_calculateContainerModules;
  ...
  parent: IModParents_mf_BoardShelf;
  addOD_M_mc_BoardHanger(index?: number): dc_mc_BoardHanger {
    return this.#internalParent.addOD_M_mc_BoardHanger(index);
  }
}
