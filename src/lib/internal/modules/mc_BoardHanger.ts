...
import { IModParents_me_BoardHanger } from '../mod-interfaces'
import { IP_part_BoardHanger_PartVarsWritable, P_part_BoardHanger } from '../parts/part_BoardHanger'
import { IModuleNonNull_mc_BoardHanger, IModParents_mc_BoardHanger, IPartParentsNonNull_mc_BoardHanger } from '../mod-interfaces'
import { VariantValidation } from '../selections'
import { IGlobalVars, GlobalVars } from '../global-vars'

import { IPartAdd_part_BoardHanger } from '../part-add-interfaces';
import { mc_BoardHanger_createBuildPlan, mc_BoardHanger_afterDataCompletion, mc_BoardHanger_manufacturerDataCompletion, mc_BoardHanger_calculateContainerModules, mc_BoardHanger_prepareContext } from '../../modules/mc_BoardHanger';

export interface cbp_mc_BoardHanger extends IModBaseProp
  , IPartAdd_part_BoardHanger {
  parent: IModParents_me_BoardHanger;
...
export interface dc_mc_BoardHanger extends IModBaseProp {
  m: OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
  seal(): IModuleNonNull_mc_BoardHanger;
  parent: IModParents_me_BoardHanger;
}

export interface pc_mc_BoardHanger extends dc_mc_BoardHanger {
...
export interface adc_base_mc_BoardHanger extends IModBaseProp {
  get m(): OD_Base[];
  setOrigin(x: number | Matrix4, y?: number, z?: number): void;
}

export interface adc_mc_BoardHanger extends adc_base_mc_BoardHanger {
}

export interface ccm_mc_BoardHanger extends adc_base_mc_BoardHanger {
}


export class OD_M_mc_BoardHanger extends OD_Base implements pc_mc_BoardHanger, dc_mc_BoardHanger
  , IModParents_mc_BoardHanger {
  constructor(parent: IModParents_me_BoardHanger, manufacturerMode?: boolean) {
    super('mc_BoardHanger', parent, parent !== undefined ? parent._manufacturerMode : manufacturerMode!);
    this.parent = parent;
    this._selfNonNull = new OD_M_mc_BoardHanger_NonNull(this);
  }
  parent: IModParents_me_BoardHanger;
  _selfNonNull: OD_M_mc_BoardHanger_NonNull;

  override variants: string[] = [];
  ...
  #manufacturerDataCompletionInternal = mc_BoardHanger_manufacturerDataCompletion;
  override afterDataCompletion(): void {
    if (this._adcFinished) return;
    this._adcFinished = true;
    this.validateSelections();
    var adc = new OD_M_mc_BoardHanger_NonNull(this);
  ...
  #prepareContextInternal = mc_BoardHanger_prepareContext;
  seal(): IModuleNonNull_mc_BoardHanger {
    this.afterDataCompletion();
    const adc = new OD_M_mc_BoardHanger_NonNull(this);
    return adc;
  }
  ...
  override calculateContainerModules(): void {
    let ccm = new OD_M_mc_BoardHanger_NonNull(this);
  ...
  addpart_BoardHanger(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_BoardHanger_PartVarsWritable {
    var c = new P_part_BoardHanger(this._selfNonNull, x, y, z, dimx, dimy, dimz, undefined);
    this.p.push(c);
    return c;
  }

  override createBuildPlan(): void {
    var cbp = new OD_M_mc_BoardHanger_NonNull(this);
    cbp.createBuildPlan();
...
class OD_M_mc_BoardHanger_NonNull implements cbp_mc_BoardHanger, adc_mc_BoardHanger, ccm_mc_BoardHanger, IPartParentsNonNull_mc_BoardHanger {
  constructor(parent: OD_M_mc_BoardHanger) {
    this.#internalParent = parent;
    this.parent = parent.parent;
  }
  #internalParent: OD_M_mc_BoardHanger;
  ...
  #createBuildPlanInternal = mc_BoardHanger_createBuildPlan;
  afterDataCompletion(): void {
    this.#afterDataCompletionInternal();
  }
  #afterDataCompletionInternal = mc_BoardHanger_afterDataCompletion;
  calculateContainerModules(): void {
    this.#calculateContainerModulesInternal();
  }
  #calculateContainerModulesInternal = mc_BoardHanger_calculateContainerModules;
  ...
  parent: IModParents_me_BoardHanger;
  addpart_BoardHanger(x: number, y: number, z: number, dimx: number, dimy: number, dimz: number): IP_part_BoardHanger_PartVarsWritable {
    return this.#internalParent.addpart_BoardHanger(x, y, z, dimx, dimy, dimz);
  }
}
