...
//#region Imports
import { cbp_mf_Fridge, dc_mf_Fridge, adc_mf_Fridge, ccm_mf_Fridge, pc_mf_Fridge } from '../internal/modules/mf_Fridge'
...
export function mf_Fridge_afterDataCompletion(this: adc_mf_Fridge): void {
	...
	try {
		...
		function AddDoor(m: any, doorHeight: number, doorPosition: number, intNmbr: number, intQty: number, startingPos: number = 0, handleInsert: boolean = true, handleMatrix = 'Center') {

			try {

				// Get the Information about the doors
				const doorInfo = JSON.parse(m.mod_Information);
				...
				if (doorInfo.configValid) {
					...
					// Add the module
					const Door = m.addOD_M_mc_Door01();
		...
		// Variables
		const FridgeInfo = JSON.parse(this.mod_FridgeInfo);
		const fridgePos = FridgeInfo.FridgePos;
		const frontHgt1 = FridgeInfo.FrontHgt1;
		const frontHgt2 = FridgeInfo.FrontHgt2;
		const frontPos2 = FridgeInfo.FrontPos2;
		const graphicId = FridgeInfo.GraphicId!
		...
		if (FridgeInfo.SplitDoor) {
			...
			const btmHandle = this.mod_HandleActivated === "Bottom" || this.mod_HandleActivated === "All";
			const topHandle = this.mod_HandleActivated === "Top" || this.mod_HandleActivated === "All";

			// Add the doors
			AddDoor(this, frontHgt1, 0, 1, 2, this.mod_Originpos[1], btmHandle, this.mod_HandlePosMatrixMultiple_matrix.FirstMatrix);
			AddDoor(this, frontHgt2, frontPos2, 2, 2, this.mod_Originpos[1] + frontPos2, topHandle, this.mod_HandlePosMatrixMultiple_matrix.SecondMatrix);
		}
		else {

			// Add the door
			AddDoor(this, frontHgt1, 0, 1, 1, this.mod_Originpos[1], true, this.mod_HandlePosMatrix);
		}
...
// ---------------------------------------------------------------
export function mf_Fridge_prepareContext(this: pc_mf_Fridge): void {
	internal_enterModulePrepareContext('mf_Fridge', this._id);
	try {
		// ###############################################################
		// ####################### CUSTOM SCRIPTS ########################
		// ###############################################################
		// CUSTOMSCRIPT_mf_Fridge_PREPARECONTEXT
...
// ---------------------------------------------------------------
export function mf_Fridge_calculateContainerModules(this: ccm_mf_Fridge): void {
	internal_enterCalculateContainerModules('mf_Fridge', this._id);
	try {
		// ###############################################################
		// ####################### CUSTOM SCRIPTS ########################
		// ###############################################################
		// CUSTOMSCRIPT_mf_Fridge_CALCULATECONTAINERMODULES
