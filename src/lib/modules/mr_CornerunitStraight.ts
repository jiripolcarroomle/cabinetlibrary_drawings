...
//#region Imports
import { cbp_mr_CornerunitStraight, dc_mr_CornerunitStraight, adc_mr_CornerunitStraight, ccm_mr_CornerunitStraight, pc_mr_CornerunitStraight } from '../internal/modules/mr_CornerunitStraight'
...
export function mr_CornerunitStraight_createBuildPlan(this: cbp_mr_CornerunitStraight): void {
	...
	try {
		...
		if (this.mod_CreatePaneltop && this.mod_CreateCeilingFiller) {
			logError(`In mr_CornerunitStraight ${this._id}: Both mod_CreatePaneltop and mod_CreateCeilingFiller are activated. This can lead to errors in the generation. Please only activate one of them.`);
		}
		...
		if (this.mod_CreateToekick && this.mod_PlinthAreaDesign != "10") {
			...
			const toekickContourBounds = {
				xMin: countertopContourBounds.xMin + (
					this.mod_CarcaseDirection === 'Right'
						? 0
						: (mod_PlinthAreaVisLeft ? LineLeft : 0)
				),
				xMid: (
					this.mod_CarcaseDirection === 'Right'
						? (
				...
				xMax: countertopContourBounds.xMax + (
					this.mod_CarcaseDirection === 'Right'
						? (mod_PlinthAreaVisRight ? LineRight : 0)
						: 0
				),
				zMin: LineBack,
...
// ---------------------------------------------------------------
export function mr_CornerunitStraight_prepareContext(this: pc_mr_CornerunitStraight): void {
	internal_enterModulePrepareContext('mr_CornerunitStraight', this._id);
	try {
		// ###############################################################
		// ####################### CUSTOM SCRIPTS ########################
		// ###############################################################
		// CUSTOMSCRIPT_mr_CornerunitStraight_PREPARECONTEXT
...
// ---------------------------------------------------------------
export function mr_CornerunitStraight_calculateContainerModules(this: ccm_mr_CornerunitStraight): void {
	internal_enterCalculateContainerModules('mr_CornerunitStraight', this._id);
	try {
		// ###############################################################
		// ####################### CUSTOM SCRIPTS ########################
		// ###############################################################
		// CUSTOMSCRIPT_mr_CornerunitStraight_CALCULATECONTAINERMODULES
