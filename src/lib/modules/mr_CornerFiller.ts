...
//#region Imports
import { cbp_mr_CornerFiller, dc_mr_CornerFiller, adc_mr_CornerFiller, ccm_mr_CornerFiller, pc_mr_CornerFiller } from '../internal/modules/mr_CornerFiller'
...
export function mr_CornerFiller_createBuildPlan(this: cbp_mr_CornerFiller): void {
  ...
  try {
    ...
    if (this.mod_CreatePaneltop && this.mod_CreateCeilingFiller) {
      logError(`In mr_CornerFiller ${this._id}: Both mod_CreatePaneltop and mod_CreateCeilingFiller are activated. This can lead to errors in the generation. Please only activate one of them.`);
    }
...
// ---------------------------------------------------------------
export function mr_CornerFiller_prepareContext(this: pc_mr_CornerFiller): void {
  internal_enterModulePrepareContext('mr_CornerFiller', this._id);
  try {
    // ###############################################################
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // CUSTOMSCRIPT_mr_CornerFiller_PREPARECONTEXT
...
// ---------------------------------------------------------------
export function mr_CornerFiller_calculateContainerModules(this: ccm_mr_CornerFiller): void {
  internal_enterCalculateContainerModules('mr_CornerFiller', this._id);
  try {
    // ###############################################################
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // CUSTOMSCRIPT_mr_CornerFiller_CALCULATECONTAINERMODULES
