...
//#region Imports
import { cbp_mr_Filler01, dc_mr_Filler01, adc_mr_Filler01, ccm_mr_Filler01, pc_mr_Filler01 } from '../internal/modules/mr_Filler01'
...
export function mr_Filler01_createBuildPlan(this: cbp_mr_Filler01): void {
  ...
  try {
    ...
    if (this.mod_CreatePaneltop && this.mod_CreateCeilingFiller) {
      logError(`In mr_Filler01 ${this._id}: Both mod_CreatePaneltop and mod_CreateCeilingFiller are activated. This can lead to errors in the generation. Please only activate one of them.`);
    }
    ...
    if (createBaseboard) {
      ...
      contourBaseboard.attributes
        .set(CONTOUR_ATTRIBUTE_OWNER_TYPE, this.modId)
        .set(CONTOUR_ATTRIBUTE_ADD_BASEBOARD, 0)
        .set(CONTOUR_ATTRIBUTE_ADD_TOEKICK, 1)
        .set('mod_Direction', this.mod_Direction)
...
// ---------------------------------------------------------------
export function mr_Filler01_prepareContext(this: pc_mr_Filler01): void {
  internal_enterModulePrepareContext('mr_Filler01', this._id);
  try {
    // ###############################################################
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // CUSTOMSCRIPT_mr_Filler01_PREPARECONTEXT
...
// ---------------------------------------------------------------
export function mr_Filler01_calculateContainerModules(this: ccm_mr_Filler01): void {
  internal_enterCalculateContainerModules('mr_Filler01', this._id);
  try {
    // ###############################################################
    // ####################### CUSTOM SCRIPTS ########################
    // ###############################################################
    // CUSTOMSCRIPT_mr_Filler01_CALCULATECONTAINERMODULES
